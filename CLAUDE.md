...
3. Line numbers in the plan are a snapshot as of commit `3d26533` and have drifted heavily since stages 1–4 landed. **Grep by anchor name** rather than trusting the numbers. Useful current anchors: `.top-bar`, `.drawer`, `.drawer-section`, `.settings-modal`, `.secondary-tabs`, `.gex-side-panel-wrap`; functions `renderChartVisibilitySection`, `renderGexSidePanel`, `syncGexPanelYAxisToTV`, `updateSecondaryTabs`, `compute_trader_stats`, `getChartVisibility`. The pre-Stage-4 anchors `.header` / `.header-top` / `.header-bottom` / `.chart-selector` / `.chart-checkbox` no longer exist.
