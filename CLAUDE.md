...
3. Line numbers in the plan are a snapshot as of commit `3d26533`. After stage 1 lands they drift — **grep by anchor name** (class names like `.header` / `.secondary-tabs`, function names like `renderGexSidePanel` / `compute_trader_stats`) rather than trusting the line numbers.
